...
// Ensure total_length guarantees a solid 5.4mm top cap (closed on top opposite to bottom thread)
total_length = (size == "8g") ? 83 : 100;
cav_depth    = (size == "8g") ? 53 : 70;
label_text   = (size == "8g") ? "8g" : "12g";

// M20.7 x 2.0 Pitch Female Thread for GIMAS Siphon Connection
module thread_2d_female(d=20.7, pitch=2.0) {
    circle(r=d/2 - 0.9, $fn=48);
    for (i=[0:11]) {
        rotate([0, 0, i*30])
        translate([d/2 - 1.0, 0])
        polygon(points=[[0, -0.65], [1.2, 0], [0, 0.65]]);
    }
}
...
module style_modern(len) {
    union() {
        cylinder(r=15, h=len);
        translate([0, 0, len-4]) cylinder(r1=15, r2=13, h=4);
        for (i=[0, 180]) {
            rotate([0, 0, i]) translate([13, 0, 0])
            hull() {
                translate([0, -4, 0]) cylinder(r=1, h=len);
                translate([0, 4, 0]) cylinder(r=1, h=len);
                translate([8, -1.5, 4]) sphere(r=2.5, $fn=24);
                translate([8, 1.5, 4]) sphere(r=2.5, $fn=24);
                translate([8, -1.5, len-5]) sphere(r=2.5, $fn=24);
                translate([8, 1.5, len-5]) sphere(r=2.5, $fn=24);
            }
        }
    }
...
module style_knurled(len) {
    union() {
        ...
        translate([0, 0, len-4]) cylinder(r1=15, r2=13, h=4);
        for (a=[0:15:345]) {
            rotate([0, 0, a]) translate([14.8, 0, 8]) cylinder(r=1.2, h=len-16, $fn=20);
        }
...
// -------------------------------------------------------------
// STYL 3: Retro Fluted (Klasyczne bruzdy w stylu vintage)
// -------------------------------------------------------------
module style_fluted(len) {
    difference() {
        ...
        for (a=[0:30:330]) {
            if (a != 0 && a != 180) {
                rotate([0, 0, a]) translate([15, 0, 8]) cylinder(r=2.5, h=len-16, $fn=20);
            }
        }
    }
}
...
// -------------------------------------------------------------
// STYL 4: Ergonomic Spiral (Futurystyczny świder chwytny)
// -------------------------------------------------------------
module style_spiral(len) {
    union() {
        ...
        intersection() {
            ...
            translate([0, 0, 8])
            linear_extrude(height=len-16, twist=160, $fn=36) {
                circle(r=15, $fn=36);
                for (a=[0:45:315]) {
...
module gimas_co2_holder(total_len, depth, stl_style, text_str) {
    difference() {
        // Outer aesthetic shell (solid cap at top z = total_len)
        if (stl_style == 1) style_modern(total_len);
        else if (stl_style == 2) style_knurled(total_len);
        else if (stl_style == 3) style_fluted(total_len);
        else if (stl_style == 4) style_spiral(total_len);
        else style_modern(total_len);

        // 1. M20.7x2 female thread at bottom mouth (height 16mm)
        translate([0, 0, -0.5])
        linear_extrude(height=16, twist=-360*(16/2.0), slices=48, $fn=48)
            thread_2d_female(20.7, 2.0);
            
        // Bottom mouth lead-in chamfer
        translate([0, 0, -1]) 
            cylinder(r1=15.0, r2=11.5, h=3.0);
